    ...
    def image_json(image, include_details: false)
      ...
      if include_details
        result.merge!({
          ...
          thumbnail_url: image.file.attached? ? url_for(image.file.representation(:thumb)) : nil,
          medium_url: image.file.attached? ? url_for(image.file.representation(:medium)) : nil,
          large_url: image.file.attached? ? url_for(image.file.representation(:large)) : nil,
